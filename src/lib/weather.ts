/**
 * 農業気象解析 & 積算温度・収穫適期予測エンジン (Open-Meteo × 気象庁/ECMWF)
 */

export interface DailyWeatherData {
  date: string;
  tempMax: number;
  tempMin: number;
  tempMean: number;
  sunshineHours: number;
  precipitation: number;
  isForecast?: boolean;
}

export interface AccumulatedWeatherResult {
  address?: string;             // 逆引き住所（都道府県・市区町村）
  dataSource: {
    provider: string;           // "Open-Meteo Global Agricultural API"
    models: string;             // "気象庁 (JMA) / ECMWF / NOAA 高解像度全球モデル"
    geoProvider: string;        // "国土地理院 / OpenStreetMap (OSM)"
  };
  latitude: number;
  longitude: number;
  startDate: string;
  endDate: string;
  totalDays: number;
  
  // 実績積算データ
  accumulatedTemp: number;      // 積算温度 (℃)
  accumulatedSunshine: number;  // 積算日照 (h)
  accumulatedRain: number;      // 積算雨量 (mm)
  
  // 収穫予測
  targetTemp: number;           // 目標積算温度
  progressPercent: number;      // 到達進捗率 (%)
  remainingTemp: number;        // 残り必要温度 (℃)
  estimatedHarvestDate: string | null; // 予想収穫適期日 (YYYY-MM-DD)
  daysUntilHarvest: number | null;     // 収穫までの予想残り日数
  
  // アラート
  alerts: Array<{
    type: 'drought' | 'heavy_rain' | 'high_temp' | 'frost' | 'harvest_ready';
    level: 'info' | 'warning' | 'danger';
    title: string;
    message: string;
  }>;
  
  // グラフ用日別データ
  dailyHistory: DailyWeatherData[];
  forecastDaily: DailyWeatherData[];
}

/**
 * 主要作物の標準目標積算温度マスタ
 */
export const CROP_ACCUMULATED_TEMP_DEFAULTS: Record<string, { targetTemp: number; baseTemp: number; description: string }> = {
  'トマト': { targetTemp: 1000, baseTemp: 10, description: '開花〜収穫まで約1000〜1100℃（大玉）' },
  'ミニトマト': { targetTemp: 900, baseTemp: 10, description: '開花〜収穫まで約800〜900℃' },
  'きゅうり': { targetTemp: 700, baseTemp: 12, description: '定植〜収穫開始まで約600〜700℃' },
  'ナス': { targetTemp: 1200, baseTemp: 12, description: '開花〜収穫まで約1100〜1200℃' },
  'ピーマン': { targetTemp: 1100, baseTemp: 12, description: '開花〜収穫まで約1000〜1100℃' },
  'トウモロコシ': { targetTemp: 950, baseTemp: 10, description: '播種〜収穫まで約900〜1000℃' },
  '枝豆': { targetTemp: 750, baseTemp: 10, description: '播種〜収穫まで約700〜800℃' },
  'キャベツ': { targetTemp: 1200, baseTemp: 5, description: '定植〜結球収穫まで約1100〜1300℃' },
  'レタス': { targetTemp: 800, baseTemp: 5, description: '定植〜収穫まで約700〜850℃' },
  'ブロッコリー': { targetTemp: 950, baseTemp: 5, description: '定植〜花蕾収穫まで約900〜1000℃' },
  'ダイコン': { targetTemp: 1000, baseTemp: 5, description: '播種〜収穫まで約900〜1100℃' },
  'ニンジン': { targetTemp: 1300, baseTemp: 5, description: '播種〜収穫まで約1200〜1400℃' },
  'ネギ': { targetTemp: 1500, baseTemp: 5, description: '定植〜収穫まで約1400〜1600℃' },
  'ほうれん草': { targetTemp: 600, baseTemp: 4, description: '播種〜収穫まで約500〜650℃' },
  'ジャガイモ': { targetTemp: 1100, baseTemp: 5, description: '萌芽〜収穫まで約1000〜1200℃' },
  'スイカ': { targetTemp: 1000, baseTemp: 12, description: '開花〜収穫まで約900〜1050℃' },
  'メロン': { targetTemp: 1100, baseTemp: 12, description: '開花〜収穫まで約1050〜1150℃' },
  'イチゴ': { targetTemp: 600, baseTemp: 5, description: '開花〜収穫まで約550〜650℃' },
  '水稲': { targetTemp: 1000, baseTemp: 10, description: '出穂〜登熟収穫まで約950〜1050℃' }
};

export function getCropTargetTemp(cropName: string): { targetTemp: number; baseTemp: number; description: string } {
  if (!cropName) {
    return { targetTemp: 1000, baseTemp: 10, description: '標準基準（1000℃）' };
  }
  for (const [key, val] of Object.entries(CROP_ACCUMULATED_TEMP_DEFAULTS)) {
    if (cropName.includes(key) || key.includes(cropName)) {
      return val;
    }
  }
  return { targetTemp: 1000, baseTemp: 10, description: '標準基準（1000℃）' };
}

/**
 * 緯度経度から住所を逆引き取得する
 */
export async function fetchReverseGeocode(lat: number, lng: number): Promise<string> {
  try {
    const res = await fetch(
      `https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${lat}&longitude=${lng}&localityLanguage=ja`
    );
    if (res.ok) {
      const data = await res.json();
      const addr = `${data.principalSubdivision || ''}${data.locality || ''}${data.city || ''}`;
      if (addr.trim()) return addr;
    }
  } catch (e) {}

  try {
    const osmRes = await fetch(
      `https://nominatim.openstreetmap.org/reverse?format=json&lat=${lat}&lon=${lng}&zoom=14&addressdetails=1`,
      { headers: { 'Accept-Language': 'ja' } }
    );
    if (osmRes.ok) {
      const osmData = await osmRes.json();
      if (osmData.display_name) {
        return osmData.display_name.split(',').slice(0, 3).reverse().join(' ');
      }
    }
  } catch (e) {}

  return `北緯${lat.toFixed(4)}°, 東経${lng.toFixed(4)}° 付近`;
}

/**
 * 緯度経度・作業開始日・終了日から積算気象データおよび収穫予測を取得・計算する
 */
export async function fetchFieldAccumulatedWeather(
  latitude: number,
  longitude: number,
  startDateStr: string,
  cropName: string,
  customTargetTemp?: number,
  customEndDateStr?: string
): Promise<AccumulatedWeatherResult> {
  const today = new Date();
  const todayStr = today.toISOString().split('T')[0];

  // 日付の正規化
  let actualStartStr = startDateStr && startDateStr.length === 10 ? startDateStr : todayStr;
  let targetEndStr = customEndDateStr && customEndDateStr.length === 10 ? customEndDateStr : todayStr;

  // 開始日が終了日より未来の場合は開始日を終了日に合わせる
  if (actualStartStr > targetEndStr) {
    actualStartStr = targetEndStr;
  }

  // Open-Meteo Forecast API は過去90日まで対応。それ以上古い場合は90日前に丸める
  const ninetyDaysAgo = new Date();
  ninetyDaysAgo.setDate(ninetyDaysAgo.getDate() - 88);
  const ninetyDaysAgoStr = ninetyDaysAgo.toISOString().split('T')[0];

  let queryStartStr = actualStartStr;
  if (queryStartStr < ninetyDaysAgoStr) {
    queryStartStr = ninetyDaysAgoStr;
  }

  // 住所の逆引きを並行取得
  const addressPromise = fetchReverseGeocode(latitude, longitude);

  // 目標積算温度の決定
  const cropSetting = getCropTargetTemp(cropName);
  const targetTemp = customTargetTemp || cropSetting.targetTemp;

  // 1. Open-Meteo API から過去実績 + 向こう14日間の予報を取得
  const forecastEndStr = addDaysStr(targetEndStr > todayStr ? targetEndStr : todayStr, 14);
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min,sunshine_duration,precipitation_sum&start_date=${queryStartStr}&end_date=${forecastEndStr}&timezone=Asia%2FTokyo`;

  let res = await fetch(url);
  if (!res.ok) {
    // フォールバック: パラメータを最小限にして今日〜14日間の予報のみ取得
    const fallbackUrl = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&daily=temperature_2m_max,temperature_2m_min,sunshine_duration,precipitation_sum&forecast_days=14&past_days=7&timezone=Asia%2FTokyo`;
    res = await fetch(fallbackUrl);
    if (!res.ok) {
      throw new Error(`気象データ取得失敗: ${res.statusText}`);
    }
  }

  const [data, resolvedAddress] = await Promise.all([res.json(), addressPromise]);
  const daily = data.daily || {};
  const dates: string[] = daily.time || [];
  const maxTemps: number[] = daily.temperature_2m_max || [];
  const minTemps: number[] = daily.temperature_2m_min || [];
  const sunshineSecs: number[] = daily.sunshine_duration || [];
  const rains: number[] = daily.precipitation_sum || [];

  const historyList: DailyWeatherData[] = [];
  const forecastList: DailyWeatherData[] = [];

  let accTemp = 0;
  let accSun = 0;
  let accRain = 0;

  for (let i = 0; i < dates.length; i++) {
    const dStr = dates[i];
    const tMax = maxTemps[i] ?? 20;
    const tMin = minTemps[i] ?? 10;
    const tMean = Math.round(((tMax + tMin) / 2) * 10) / 10;
    const sunHours = Math.round(((sunshineSecs[i] ?? 0) / 3600) * 10) / 10;
    const rain = Math.round((rains[i] ?? 0) * 10) / 10;

    const isPastOrToday = dStr <= targetEndStr;

    const dayObj: DailyWeatherData = {
      date: dStr,
      tempMax: tMax,
      tempMin: tMin,
      tempMean: tMean,
      sunshineHours: sunHours,
      precipitation: rain,
      isForecast: !isPastOrToday
    };

    if (isPastOrToday) {
      if (dStr >= actualStartStr) {
        historyList.push(dayObj);
        accTemp += tMean;
        accSun += sunHours;
        accRain += rain;
      }
    } else {
      forecastList.push(dayObj);
    }
  }

  accTemp = Math.round(accTemp * 10) / 10;
  accSun = Math.round(accSun * 10) / 10;
  accRain = Math.round(accRain * 10) / 10;

  // 収穫適期予測計算
  const remainingTemp = Math.max(0, targetTemp - accTemp);
  const progressPercent = Math.min(100, Math.round((accTemp / targetTemp) * 100));

  let estimatedHarvestDate: string | null = null;
  let daysUntilHarvest: number | null = null;

  if (accTemp >= targetTemp) {
    estimatedHarvestDate = todayStr;
    daysUntilHarvest = 0;
  } else {
    // 予報気温を使ってシミュレーション
    let tempSum = accTemp;
    let foundDate: string | null = null;
    let daysCount = 0;

    for (const fDay of forecastList) {
      tempSum += fDay.tempMean;
      daysCount++;
      if (tempSum >= targetTemp) {
        foundDate = fDay.date;
        break;
      }
    }

    if (!foundDate) {
      const avgRecentDailyTemp = historyList.length > 0
        ? historyList.slice(-7).reduce((sum, d) => sum + d.tempMean, 0) / Math.min(7, historyList.length)
        : 20;
      const safeDailyTemp = Math.max(1, avgRecentDailyTemp);
      const remainingDays = Math.ceil(remainingTemp / safeDailyTemp);
      
      estimatedHarvestDate = addDaysStr(todayStr, remainingDays);
      daysUntilHarvest = remainingDays;
    } else {
      estimatedHarvestDate = foundDate;
      daysUntilHarvest = daysCount;
    }
  }

  // アラート判定
  const alerts: AccumulatedWeatherResult['alerts'] = [];

  if (progressPercent >= 100) {
    alerts.push({
      type: 'harvest_ready',
      level: 'info',
      title: '🎉 収穫適期に到達！',
      message: `目標積算温度（${targetTemp}℃）に到達しました。色づき・実入りを確認し、収穫適期を迎えています。`
    });
  } else if (progressPercent >= 90) {
    alerts.push({
      type: 'harvest_ready',
      level: 'info',
      title: '🌾 まもなく収穫期',
      message: `目標積算温度の90%を超過（あと約${daysUntilHarvest}日）。収穫・出荷準備の目安時期です。`
    });
  }

  // 土壌乾燥アラート
  const recentHistory = historyList.slice(-4);
  const recentRainSum = recentHistory.reduce((sum, d) => sum + d.precipitation, 0);
  if (recentHistory.length >= 3 && recentRainSum === 0) {
    alerts.push({
      type: 'drought',
      level: 'warning',
      title: '💧 土壌乾燥注意・潅水推奨',
      message: '直近4日間に降雨が記録されていません。圃場の土壌水分を確認し、必要に応じて潅水を行ってください。'
    });
  }

  // 高温多湿アラート
  const recentAvgTemp = recentHistory.length > 0 ? recentHistory.reduce((sum, d) => sum + d.tempMean, 0) / recentHistory.length : 0;
  if (recentAvgTemp >= 28 && recentRainSum > 5) {
    alerts.push({
      type: 'high_temp',
      level: 'warning',
      title: '⚠️ 高温多湿・病害虫注意',
      message: '高温かつ多湿な環境が続いています。軟腐病・疫病・ハダニ等の発生予防のため防除を点検してください。'
    });
  }

  return {
    address: resolvedAddress,
    dataSource: {
      provider: 'Open-Meteo Global Agricultural API',
      models: '気象庁 (JMA 高解像度数値予報MSM/GSM) ＆ 欧州ECMWF / 米国NOAA全球気象モデル統合解析',
      geoProvider: '国土地理院 ＆ OpenStreetMap (OSM) 地理座標データベース'
    },
    latitude,
    longitude,
    startDate: actualStartStr,
    endDate: targetEndStr,
    totalDays: historyList.length,
    accumulatedTemp: accTemp,
    accumulatedSunshine: accSun,
    accumulatedRain: accRain,
    targetTemp,
    progressPercent,
    remainingTemp,
    estimatedHarvestDate,
    daysUntilHarvest,
    alerts,
    dailyHistory: historyList,
    forecastDaily: forecastList
  };
}

function addDaysStr(dateStr: string, days: number): string {
  const d = new Date(dateStr);
  d.setDate(d.getDate() + days);
  return d.toISOString().split('T')[0];
}
