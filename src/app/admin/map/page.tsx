...
async function autoTranslate(text: string) {
  if (!text) return { vi: '', id: '', zh: '', en: '', si: '', km: '' };
  try {
    const langs = ['vi', 'id', 'zh-CN', 'en', 'si', 'km'];
    ...
    return {
      ...
      si: results.find(r => r.lang === 'si')?.text || '',
      km: results.find(r => r.lang === 'km')?.text || ''
    };
  } catch (error) {
    console.error("Translation error:", error);
    return { vi: '', id: '', zh: '', en: '', si: '', km: '' };
  }
}
