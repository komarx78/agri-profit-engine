...
export default function AccountingExportPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-emerald-50/70 p-4 rounded-xl border border-emerald-200 flex items-start gap-2.5">
        <Info className="w-4 h-4 text-emerald-600 shrink-0 mt-0.5" />
        <p className="text-xs text-emerald-900 font-bold leading-relaxed">
          【お知らせ】売上の計上方法が「請求書発行ベース（掛売上）」に対応しております。<br />
          <span className="font-normal text-emerald-700">自動的に「対象月・出荷先ごとの請求書単位」に合算され、月末日付で1本だけ売上仕訳が作成されます。</span>
        </p>
      </div>

      {message && (
        <div className={`p-3 rounded-xl text-xs font-bold text-center ${message.includes('エラー') || message.includes('ありません') ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-900'}`}>
          {message}
        </div>
      )}
        ...
        <div className="md:col-span-1 space-y-4">
          <div className="bg-white p-5 rounded-2xl shadow-xs border border-slate-200">
            <h2 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
              <Calendar className="w-4 h-4 text-emerald-600" /> 抽出期間
            </h2>
            <div className="space-y-3">
              <div>
                ...
                <input
                  ...
                  className="w-full p-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              <div>
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-400 text-white py-3 px-4 rounded-xl font-bold text-xs shadow-md transition-all flex items-center justify-center gap-2"
          >
            {isLoading ? '生成中...' : (
              <>
          ...
          <div className="bg-white p-5 rounded-2xl shadow-xs border border-slate-200">
            ...
            <div className="space-y-4">
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <h3 className="font-bold text-xs text-slate-700 mb-2 border-b border-emerald-200 inline-block pb-0.5">売上 (請求書発行ベース) の仕訳</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 mb-1">借方科目 (例: 売掛金)</label>
                    <input type="text" name="salesDebit" value={accounts.salesDebit} onChange={handleAccountChange} className="w-full p-1.5 border border-slate-200 rounded-lg text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 mb-1">貸方科目 (例: 売上高)</label>
                    <input type="text" name="salesCredit" value={accounts.salesCredit} onChange={handleAccountChange} className="w-full p-1.5 border border-slate-200 rounded-lg text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none" />
                  </div>
                </div>
              </div>

              <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                <h3 className="font-bold text-xs text-slate-700 mb-2 border-b border-emerald-200 inline-block pb-0.5">資材購入時の仕訳 (コスト)</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 mb-1">借方科目 (例: 消耗品費, 肥料費)</label>
                    <input type="text" name="materialDebit" value={accounts.materialDebit} onChange={handleAccountChange} className="w-full p-1.5 border border-slate-200 rounded-lg text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-500 mb-1">貸方科目 (例: 現金, 買掛金)</label>
                    <input type="text" name="materialCredit" value={accounts.materialCredit} onChange={handleAccountChange} className="w-full p-1.5 border border-slate-200 rounded-lg text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none" />
                  </div>
                </div>
              </div>
            </div>
