...
export default function MonthlyExpensesPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const tenantId = await getCurrentTenantId();
      if (!tenantId) throw new Error('テナントIDが特定できませんでした');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-emerald-50/70 border border-emerald-200 rounded-xl p-4 flex gap-3 text-emerald-900 text-xs sm:text-sm">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <input 
              ...
              className="px-3 py-1.5 border border-slate-300 rounded-lg font-bold text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white"
            />
          </div>
          <button 
          ...
          <form onSubmit={handleSave} className="p-6 space-y-6">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div className="relative">
                  <input 
                    ...
                    placeholder="0"
                    className="w-full pl-3 pr-8 py-2.5 bg-white border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 font-bold text-right text-base"
                  />
            ...
            <div className="pt-2 border-t border-slate-100 flex justify-end">
              <button 
                type="submit" 
                disabled={isSaving}
                className="px-6 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow-xs transition-colors flex items-center gap-2"
              >
