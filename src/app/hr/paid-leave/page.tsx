...
import { 
  ...
  RefreshCw, Check, ArrowRight, Zap, Info, ChevronRight
} from 'lucide-react';
...
export function calculateStatutoryLeave(
  ...
  const isFullTime = employmentType === '正社員' || weeklyDays >= 5;
...
export default function PaidLeavePage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (workerIds.length > 0) {
        ...
        // 3. 勤怠打刻ログから有給打刻を取得（有給消化自動集計）
        const { data: attData } = await supabase
          .from('attendance_logs')
          .select('id, worker_id, date, status')
          .in('worker_id', workerIds)
          .in('status', ['paid_leave', 'half_paid_am', 'half_paid_pm']);
  ...
  const analyzedWorkers = useMemo(() => {
    return workers.map(emp => {
      const isDispatch = emp.type === '派遣' || emp.employment_type === '派遣';
      const weeklyDays = emp.weekly_working_days ?? emp.weekly_days ?? 5;
      const empType = emp.employment_type || emp.type || '正社員';

      // 法定自動計算
      const statutory = calculateStatutoryLeave(emp.join_date, empType, weeklyDays);

      // 有給消化日数の自動集計（打刻ログ ＋ 承認済み申請）
      let usedDays = 0;

      // 1. 打刻ログからの集計
      const empAttLogs = attendanceLogs.filter(l => l.worker_id === emp.id);
      empAttLogs.forEach(l => {
        if (l.status === 'paid_leave') usedDays += 1.0;
        else if (l.status === 'half_paid_am' || l.status === 'half_paid_pm') usedDays += 0.5;
      });

      // 2. 承認済み申請からの集計（打刻にない日を安全に加算）
      const empApprovedReqs = leaveRequests.filter(
        r => r.worker_id === emp.id && r.status === '承認' && r.type === '有給休暇'
      );
      empApprovedReqs.forEach(r => {
        if (r.start_date && r.end_date) {
          ...
          // 打刻と重複していないか簡易判定
          const hasAttOverlap = empAttLogs.some(l => l.date >= r.start_date && l.date <= r.end_date);
          if (!hasAttOverlap) {
            usedDays += diffDays;
          }
        }
  ...
  }, [workers, leaveRequests, attendanceLogs]);

  // 全社サマリー集計
  const summary = useMemo(() => {
    const valid = analyzedWorkers.filter(w => !w.isDispatch);
    ...
    const obligatedWorkers = valid.filter(w => w.isObligated);
    const alertWorkers = obligatedWorkers.filter(w => !w.isObligationSatisfied);

    return {
      totalWorkers: valid.length,
      dispatchWorkers: analyzedWorkers.filter(w => w.isDispatch).length,
      totalGranted,
      totalUsed,
      totalRemaining,
      usageRate,
      obligatedCount: obligatedWorkers.length,
      alertCount: alertWorkers.length
    };
  }, [analyzedWorkers]);

  // フィルタリングされた従業員一覧
  const filteredWorkers = useMemo(() => {
    return analyzedWorkers.filter(w => {
      if (filterType === 'alert_only') return w.isObligated && !w.isObligationSatisfied;
      if (filterType === 'fulltime') return !w.isDispatch && (w.empType === '正社員' || w.weeklyDays >= 5);
      if (filterType === 'part') return !w.isDispatch && w.empType !== '正社員' && w.weeklyDays < 5;
      return true;
    });
  }, [analyzedWorkers, filterType]);

  // 全員の法定有給を一括自動反映
  const handleAutoApplyAllStatutory = async () => {
    const targets = analyzedWorkers.filter(w => !w.isDispatch && w.join_date);
    if (targets.length === 0) {
  ...
  const handleSaveBalance = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(`💾 ${editingWorker.name} さんの有給設定を保存しました！`);
      fetchData();
    } catch (err: any) {
      console.error(err);
  ...
  const handleExportCsv = () => {
    ...
    const rows = analyzedWorkers.map(w => [
      w.id,
      w.name,
  ...
  return (
    <AdminOnlyGuard>
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            <div className="bg-slate-100 p-1 rounded-xl flex items-center border border-slate-200">
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${
                  activeTab === 'balance' ? 'bg-white text-amber-600 shadow-2xs' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                有給残日数・算定台帳
              </button>
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-xs font-black transition-all ${
                  activeTab === 'requests' ? 'bg-white text-amber-600 shadow-2xs' : 'text-slate-500 hover:text-slate-800'
                }`}
              >
                休暇申請・承認履歴
              </button>
            ...
            <button 
              ...
              className="flex items-center gap-1.5 bg-white border border-slate-200 text-slate-700 hover:text-amber-700 hover:border-amber-300 px-3.5 py-2 rounded-xl font-bold text-xs hover:bg-amber-50/50 shadow-2xs transition-all"
            >
            ...
            {activeTab === 'balance' && (
              <button 
                ...
                className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white px-4 py-2 rounded-xl font-black text-xs shadow-md shadow-amber-500/20 hover:shadow-lg transition-all disabled:opacity-50"
              >
            ...
            {activeTab === 'requests' && (
              <button 
                ...
                className="flex items-center gap-1.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-2 rounded-xl font-black text-xs shadow-md shadow-amber-500/20 hover:shadow-lg transition-all"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-2xs flex flex-col justify-between">
            ...
            <div className="mt-3 flex items-baseline gap-2">
              <span className="text-3xl font-black text-slate-800">{summary.totalWorkers}</span>
              <span className="text-xs font-bold text-slate-500">名</span>
              {summary.dispatchWorkers > 0 && (
                <span className="text-[11px] font-bold text-slate-400 ml-auto">
                  (派遣・対象外: {summary.dispatchWorkers}名)
                </span>
              )}
            </div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-200/50 text-[11px] font-bold text-slate-500 flex items-center justify-between">
              ...
              {summary.alertCount > 0 ? (
                <button 
                  ...
                  className="text-rose-600 font-black hover:underline flex items-center gap-0.5"
                >
        ...
        {/* メインコンテンツエリア */}
        <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[450px]">
          ...
          {activeTab === 'balance' && (
            <div className="p-4 bg-slate-50/80 border-b border-slate-200 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                {[
                  { id: 'all', label: 'すべて表示' },
                  { id: 'alert_only', label: `⚠️ 年5日未達 (${summary.alertCount})`, badgeColor: 'bg-rose-500 text-white' },
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold transition-all border ${
                      filterType === tab.id
                        ? 'bg-amber-500 border-amber-500 text-white shadow-2xs font-black'
              ...
              <div className="text-xs font-bold text-slate-400 flex items-center gap-1.5">
                ...
                <span>勤怠打刻（有給・半休）および承認済み申請と自動連動中</span>
              </div>
            </div>
          )}
                      ...
                      {filteredWorkers.length === 0 ? (
                        ...
                        return (
                          <tr key={emp.id} className={`hover:bg-amber-50/20 transition-colors ${emp.isDispatch ? 'bg-slate-50/50 opacity-60' : ''}`}>
                            ...
                            {/* 2. 入社日・勤続期間 */}
                            <td className="p-4">
                              <div className="text-xs font-bold text-slate-700">{emp.join_date || '未設定'}</div>
                            ...
                            <td className="p-4 text-center">
                              {emp.isDispatch ? (
                                ...
                                <div>
                                  ...
                                  {st.statutoryGrant !== emp.balance && emp.join_date && (
                            ...
                            <td className="p-4 text-center">
                              <div className="flex items-center justify-center gap-1.5">
                                {!emp.isDispatch && emp.join_date && st.statutoryGrant !== emp.balance && (
                                  <button
                                    type="button"
                                ...
                                <button 
                                  ...
                                  className="px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-black text-xs rounded-xl transition-colors flex items-center gap-1"
                                >
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {leaveRequests.length === 0 ? (
                        ...
                        <tr key={req.id} className="hover:bg-slate-50">
                          ...
                          <td className="p-4 font-black text-slate-800">{req.workers?.name || '-'}</td>
                          <td className="p-4">
                            <span className={`px-2 py-0.5 rounded text-xs font-black ${
                              req.type === '有給休暇' ? 'bg-amber-100 text-amber-800' : 'bg-slate-100 text-slate-700'
                            }`}>
                              {req.type}
                            </span>
                          </td>
                          <td className="p-4 font-bold text-slate-700 text-sm">{req.start_date} 〜 {req.end_date}</td>
                          <td className="p-4 text-xs text-slate-600 font-medium">{req.reason || '-'}</td>
                          <td className="p-4 text-center">
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden border border-slate-200">
              <div className="px-6 py-4 bg-gradient-to-r from-amber-500 to-orange-500 text-white flex items-center justify-between">
                ...
                <button onClick={() => setIsRequestModalOpen(false)} className="text-white/80 hover:text-white">
                  <XCircle className="w-5 h-5" />
                </button>
              </div>
              <form onSubmit={handleSubmitRequest} className="p-6 space-y-4">
                <div>
                  ...
                  <select 
                    required 
                    value={requestForm.worker_id} 
                    onChange={e => setRequestForm({...requestForm, worker_id: e.target.value})} 
                    className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm"
                  >
                    <option value="">選択してください</option>
                    {workers.filter(w => w.type !== '派遣' && w.employment_type !== '派遣').map(w => (
                ...
                <button 
                  type="submit" 
                  className="w-full bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-black py-3 rounded-xl shadow-md transition-all mt-2 text-sm"
                >
                  登録する（即時承認）
                </button>
              </form>
          ...
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4 animate-in fade-in">
            <div className="bg-white rounded-3xl w-full max-w-lg shadow-2xl overflow-hidden border border-slate-200">
              <div className="px-6 py-4 bg-slate-900 text-white flex items-center justify-between">
                <div>
                  <h3 className="font-black text-base">{editingWorker.name} の有給設定・手動調整</h3>
              ...
              {/* 法定参考値サマリー */}
              {(() => {
                const weeklyDays = Number(editingWorker.weekly_working_days) || Number(editingWorker.weekly_days) || 5;
                const empType = editingWorker.employment_type || '正社員';
                const st = calculateStatutoryLeave(editingWorker.join_date, empType, weeklyDays);

                return (
                  <div className="p-6 space-y-4">
                    {editingWorker.join_date ? (
                      <div className="bg-amber-50/70 p-4 rounded-2xl border border-amber-200 flex items-center justify-between">
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-white rounded-xl text-xs font-black shadow-2xs transition-all flex items-center gap-1"
                        >
                    ...
                    <form onSubmit={handleSaveBalance} className="space-y-4">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <select 
                            value={editingWorker.employment_type || '正社員'} 
                            onChange={e => setEditingWorker({...editingWorker, employment_type: e.target.value})} 
                            className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm"
                          >
                        ...
                        <div>
                          ...
                          <select 
                            value={editingWorker.weekly_working_days ?? editingWorker.weekly_days ?? 5} 
                            onChange={e => setEditingWorker({...editingWorker, weekly_working_days: Number(e.target.value)})} 
                            className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm"
                          >
                      ...
                      <div>
                        ...
                        <input 
                          type="date" 
                          value={editingWorker.join_date || ''} 
                          onChange={e => setEditingWorker({...editingWorker, join_date: e.target.value})} 
                          className="w-full p-2.5 border border-slate-200 rounded-xl font-bold text-sm" 
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-2">
                        <div>
                          ...
                          <input 
                            ...
                            step="0.5" 
                            min="0"
                            value={editingWorker.paid_leave_carryover ?? 0} 
                            onChange={e => setEditingWorker({...editingWorker, paid_leave_carryover: e.target.value})} 
                        ...
                        <div>
                          ...
                          <input 
                            ...
                            step="0.5" 
                            min="0"
                            value={editingWorker.paid_leave_balance ?? 0} 
                            onChange={e => setEditingWorker({...editingWorker, paid_leave_balance: e.target.value})} 
                      ...
                      <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2">
                        ...
                        <button 
                          type="submit" 
                          className="px-5 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-black text-xs rounded-xl shadow-md transition-all"
                        >
                          設定を保存
                        </button>
                      </div>
