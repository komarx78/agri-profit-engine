...
export default function CloudPortalHub({ onSwitchToWork }: CloudPortalHubProps) {
  ...
  const totalTasks = pendingOrdersCount + attendanceErrorCount + pendingLeavesCount;
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        {onSwitchToWork && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-2xl font-black text-xs transition-all shadow-md self-start sm:self-auto"
          >
            <Layout className="w-4 h-4" />
            <span>📱 現場ホーム（打刻・タスク）を開く</span>
          </button>
        )}
      </div>
        ...
        <div className="space-y-3">
          ...
          {pendingLeavesCount > 0 && (
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 bg-indigo-50/80 border border-indigo-200 rounded-2xl">
              <div className="flex items-start sm:items-center gap-3.5">
                <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center shadow-sm text-indigo-600 shrink-0 border border-indigo-100">
                  <Users className="w-5 h-5 text-indigo-600" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-black bg-indigo-600 text-white px-2 py-0.2 rounded-md">
                      有給申請
                    </span>
                    <p className="font-black text-slate-800 text-sm">
                      {pendingLeavesCount}件の未承認の休暇申請があります
                    </p>
                  </div>
                  <p className="text-xs text-slate-500 font-bold mt-0.5">
                    スタッフからの有給・休暇申請が届いています。内容を確認して承認してください。
                  </p>
                </div>
              </div>
              <Link 
          ...
          {totalTasks === 0 && !loadingTasks && (
            <div className="flex items-center gap-3.5 p-5 bg-emerald-50/60 border border-emerald-200 rounded-2xl text-emerald-900">
              ...
              <div>
                ...
                <p className="text-xs text-emerald-700/80 mt-0.5">勤怠打刻エラー、未確定受注、有給申請等はすべて正常に処理されています。</p>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        {onSwitchToWork ? (
          <button 
            type="button"
            onClick={onSwitchToWork} 
            className="group bg-gradient-to-br from-indigo-50 to-white rounded-3xl p-6 shadow-sm border border-indigo-100 hover:shadow-md hover:border-indigo-300 transition-all flex flex-col text-left relative overflow-hidden"
          >
          ...
          <Link href="/portal?tab=home" className="group bg-gradient-to-br from-indigo-50 to-white rounded-3xl p-6 shadow-sm border border-indigo-100 hover:shadow-md hover:border-indigo-300 transition-all flex flex-col relative overflow-hidden">
            ...
            <h3 className="text-lg font-black text-indigo-900 mb-2 group-hover:text-indigo-700 transition-colors">📱 出退勤・現場ポータル</h3>
            <p className="text-sm font-bold text-indigo-700/70 flex-1 leading-relaxed">
              出勤・退勤の打刻、直近スケジュール、社内掲示板、外国人スタッフ向け動画マニュアルを確認できます。
            </p>
          </Link>
        )}
        ...
        <Link href="/admin/cultivation-schedule" className="group bg-white rounded-3xl p-6 shadow-sm border border-slate-200 hover:shadow-md hover:border-teal-300 transition-all flex flex-col relative overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] font-black bg-teal-100 text-teal-700 px-2 py-0.5 rounded-md">計画・予実</span>
          </div>
          <h3 className="text-lg font-black text-slate-800 mb-2 group-hover:text-teal-600 transition-colors">📊 栽培・予実管理表</h3>
          <p className="text-sm font-bold text-slate-500 flex-1 leading-relaxed">
            作付計画ごとの売上・資材費・労働時間の収支分析、育苗スケジュール（ロス率計算）を統合管理。
          </p>
        </Link>
        ...
        <Link href="/hr" className="group bg-white rounded-3xl p-6 shadow-sm border border-slate-200 hover:shadow-md hover:border-sky-300 transition-all flex flex-col relative overflow-hidden">
          ...
          <p className="text-sm font-bold text-slate-500 flex-1 leading-relaxed">
            従業員の出退勤打刻状況、休憩・残業時間の自動集計、有給休暇管理、月次タイムカードのCSV出力。
          </p>
        </Link>
        ...
        <Link href="/admin/masters" className="group bg-white rounded-3xl p-6 shadow-sm border border-slate-200 hover:shadow-md hover:border-slate-400 transition-all flex flex-col relative overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] font-black bg-slate-100 text-slate-700 px-2 py-0.5 rounded-md">マスタ設定</span>
          </div>
          <h3 className="text-lg font-black text-slate-800 mb-2 group-hover:text-slate-900 transition-colors">⚙️ マスタ管理センター</h3>
          <p className="text-sm font-bold text-slate-500 flex-1 leading-relaxed">
            作目、圃場、作業者、資材・農薬、販路・単価などの基本マスタ設定を一元管理。
          </p>
        </Link>
