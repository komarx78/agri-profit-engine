...
// workersカードの親のgridの中に安全に追加する
// workersカードの最後の部分を探す
const workersMarker = `type="workers" inputRef={fileInputRefWorkers} />`;
let insertPos = content.indexOf(workersMarker);
if (insertPos !== -1) {
  // その次の </div>\n              </div> を見つける
  insertPos = content.indexOf('</div>', insertPos);
  insertPos = content.indexOf('</div>', insertPos + 1) + 6;
  
  content = content.substring(0, insertPos) + '\n' + departmentsCardHtml + content.substring(insertPos);
}

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully re-inserted departments card');
